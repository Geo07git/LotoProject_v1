...
models = {
    ...
    'SVM': SVC(random_state=SEED, probability=True, verbose=0),
    #'CatBoost': CatBoostClassifier(verbose=1, random_state=SEED),
...
def predict_final_xgboost():
    ...
    final_prediction = [num for num, freq in num_freq.most_common(20)]
...
if user_numbers:
    ...
    matches = {i: 0 for i in range(7, 16)}  # Dicționar pentru a ține numărul de potriviri pentru fiecare valoare între 7 și 15
    ...
    for _, row in data.iterrows():
        ...
        if match_count >= 7:  # Verifică doar potrivirile de la 7 în sus
            if match_count <= 15:  # Asigură-te că nu depășești 15
                matches[match_count] += 1

    # Afișarea rezultatelor
    st.write("## Rezultate")
    for i in range(7, 11):  # Afișează rezultatele pentru potrivirile între 7 și 15
